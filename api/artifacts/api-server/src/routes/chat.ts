...
import { resolveOrgIdFromRequest } from "../lib/resolveOrg.js";
import { getOrgDoc } from "../lib/orgCache.js";

import { chatRateLimiter, chatCooldown } from "../middlewares/rateLimiter.js";
import { dynamicTenantCors } from "../middlewares/cors.js";
import { internalJwt } from "../middlewares/internalJwt.js";
...
router.post("/chat", dynamicTenantCors, internalJwt, chatRateLimiter, chatCooldown, async (req: Request, res: Response): Promise<void> => {
  ...
  const activePlans = ["start", "scale", "pro", "trial"];
  if (!activePlans.includes(orgData.plan)) {
    res.status(403).json({ error: "This form has been disabled" });
    return;
  }
